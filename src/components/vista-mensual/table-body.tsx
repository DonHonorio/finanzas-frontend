...
export function TableBody({ table, onView, onEdit, compact = false }: Props) {
                    ...
                    <tr key={row.id}
                        ...
                        {row.getVisibleCells().map(cell => (
                            <td
                                key={cell.id}
                                className={cn(
                                    // compact reduce padding y tamaño de fuente para que quepan los valores en tablet
                                    compact ? 'px-1 py-1.5 text-xs' : 'p-3 text-[16px]',
